...
// Base URL from .env (fallback if not found)
const BASE_URL = process.env.REACT_APP_API_URL || "https://eventure-gzbnqpst6-eventures-projects.vercel.app";

function EventCard(props) {
    const navigate = useNavigate();

    // Extracting event details from props
    const { _id, name, date, place, club, description, slots, startTime, endTime, registeredUsers } = props.obj;
    let year = date.slice(0, 4);          // "2025"
    let month = date.slice(5, 7).padStart(2, "0"); // "09"
    let day = date.slice(8, 10).padStart(2, "0");  // "05"

    const user = localStorage.getItem("user"); // Get current user

    // Function to book event
    const Book = () => {
        Axios.get(`${BASE_URL}/eventRoute/check-user/${user}`)
            .then((res) => {
                if (res.status === 200 && res.data != null) {
                    ...
                    if (check) {
                        ...
                        Axios.all([
                            // Updating user
                            Axios.put(`${BASE_URL}/eventRoute/update-user/${res.data._id}`, userData)
                                .then((updateResponse) => {
                            ...
                            // Updating event
                            Axios.put(`${BASE_URL}/eventRoute/update-event/${_id}`, eventData)
    ...
    const viewRegisteredUsers = () => {
        setDescription(
            <table className='userTable' border="1" cellSpacing="0" cellPadding="5">
                <thead>
                    <tr>
    ...
    const deleteEvent = () => {
        Axios.all([
            Axios.delete(`${BASE_URL}/eventRoute/delete-event/${_id}`)
                .then((res) => {
                    if (res.status === 200) {
            ...
            Axios.get(`${BASE_URL}/eventRoute/user-list`)
                .then((userResponse) => {
                    if (userResponse.status === 200) {
                        ...
                        for (let i = 0; i < collectedUsers.length; i++) {
                            ...
                            Axios.put(`${BASE_URL}/eventRoute/update-user/${collectedUsers[i]._id}`, userData)
                                .then((updateResponse) => {
                                    if (updateResponse.status === 200)
    ...
    const closeDescription = () => {
        ...
        setDescButton(
            <button className='cardButton' style={{ backgroundColor: "wheat" }} onClick={viewDescription}>
                View Description
            </button>
        );
    };
    ...
    const viewDescription = () => {
        setDescription(<Card.Text style={{ fontSize: "1.75vw", fontWeight: "bolder" }}>{description}</Card.Text>);
        setDescButton(
            <button className='cardButton' style={{ backgroundColor: "wheat" }} onClick={closeDescription}>
                Close Description
            </button>
        );
    };
